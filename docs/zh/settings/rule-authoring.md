...
自 v3.2.4 起，求值接口时抛异常的规则会被收集起来，并在导出运行结束时以**一条警告通知**的形式呈现，而不再是静默跳过该接口。如果导出结果比预期少，请先查看这条通知；每次异常的堆栈信息记录在 IDE 日志中（**Help → Show Log in…**）。

常见原因是 Groovy 脚本调用了当前上下文类型上不存在的 API —— 例如把 `it.isStatic()` 方法写成了属性 `it.static`，或在参数上下文上调用 `canonicalText()`（它返回的是元素路径，而非参数类型）。各类上下文可用的方法请参见 [`it`](./tools/it)。

正常的规则不受影响：没有失败就不会有通知。在导出运行之外执行的 API Explorer 扫描只会逐条记录日志，不会弹出通知。

### 提议在暂存前会先试运行

在向你展示之前，提议的 Groovy 值和过滤器会针对有代表性的 PSI 上下文执行一次。**所有**上下文类型都编译失败或 API 不存在时，该提议会被直接拦截；只有部分上下文类型失败时，失败信息会作为审查提示附在提议上，由你决定。

### 规则文件中的非 ASCII 文本

规则文件以 UTF-8 读写。如果规则编辑器中的中文或其他非 ASCII 文本出现乱码，请将文件重新保存为 UTF-8 —— 早期版本在读写时可能错误地使用了平台默认编码。

### 规则文件按项目解析

助手按**文件名**（例如 `security.rules`）寻址规则文件，并相对于项目目录解析。读取受跟踪的 `.easyapi/` 目录之外的文件时，仍然需要你通过内联的同意/拒绝提示进行授权。

## 工作流和多应用说明

新版规则编写 agent 可以检查解析后的类型名称和方法体，在对话失败时重试，并将相互独立的任务隔离到子 agent 中。这些能力可以改善检测结果，但规则在保存前仍然需要你审查并明确批准。

当一个工作区包含多个应用时，应为每个应用提出完整的规则组合：Host、生产令牌的响应脚本、消费者请求头以及所使用的环境变量。使用规范化后的模块名或应用名为 Host 和令牌变量添加命名空间，避免一个服务覆盖另一个服务的环境值。写规则前先确定应用身份，名称冲突时请求确认，并且不要在规则值中直接写入凭据。
