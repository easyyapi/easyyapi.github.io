...
- **`yapi.project`** — **YApi 项目 token**，决定 API 导出到哪个 YApi 项目。`module` 和 `project` 是 `yapi.project` 的旧别名，**不能**用来设置文件夹。
...
## 使用 `@module` / `@project` 标签（仅用于 YApi 项目）

`@module` 和 `@project` 标签会被解析为 `yapi.project` 规则，用于选择 **API 所属的 YApi 项目**，**不是**用于把 API 分组到文件夹：
