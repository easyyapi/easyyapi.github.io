...
::: tip 无需手动填写 Token
导出 API 时**不需要**事先在设置中填入 Token（或服务器地址）。当插件向 YApi
导出时，会自动弹出对话框，要求输入 YApi 服务器地址以及目标模块对应的项目
Token。在其中输入后，插件会自动保存，下次导出时直接复用。

上方的 **Tokens** 输入框只是为了方便一次性预填多个模块的 Token，可选项。
:::
